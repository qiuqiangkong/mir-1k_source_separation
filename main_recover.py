...
# hyper params
n_time=11
md = serializations.load(cfg.md_fd+'/md10_epochs.p')
fe_fd = cfg.fe_fft_fd
    ...
    if na.endswith('.wav') and not na.endswith('lyrics.wav') and not na.endswith('lyric.wav'):
        if len(na.split('_'))==3:
            ...
            if gt_mix is not None:
                ...
                print "Left channel: sdr:", sdr, "sir:", sir, "sar:", sar, "perm:", perm
                sdr, sir, sar, perm = bss_eval_sources(s_gt_right, s_pred_right)
                print "Right channel: sdr:", sdr, "sir:", sir, "sar:", sar, "perm:", perm
                ...
                print "Separated result of", na, "has been written to", cfg.results_fd
                pause