...
def get_xy(fe_fd, n_time, tr_phase):
    X_list, y_left_list, y_right_list = GetListData(fe_fd, tr_phase)
    
    def _get_3d(Xlist, n_time):
        X3d_all = []
        for X in Xlist:
            n_in = X.shape[1]
            pad = np.zeros((n_time-1, n_in))
            X = np.concatenate((pad, X))
            X3d = mat_2d_to_3d(X, n_time, 1)
            X3d_all.append(X3d)
        X3d_all = np.concatenate(X3d_all, axis=0)
        return X3d_all
    
    X3d_all = _get_3d(X_list, n_time)
    y_left_all = _get_3d(y_left_list, n_time)
    y_right_all = _get_3d(y_right_list, n_time)
    return X3d_all, y_left_all, y_right_all
